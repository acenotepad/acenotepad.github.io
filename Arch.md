...
### 2. Before entering the VM, go into your settings and update it from loading the BIOS to loading UEFI. If done correctly, the command cat /sys/firmware/efi/fw_platform_size will return a value of 64 instead of "file does not exist". Check this as soon as you boot up Arch for the first time as shown in step 4. 
...
#### 6.4 See the image below for example commands that I used when I partitioned my disk.
...
### 9. Mounting:  
#### 9.1. Recall the three partitions you have. Each one will run a different command to mount the filesystem that is labeled by partition below. You will replace these names with the real names of your partitions.
#### 9.2. **mount /dev/*root-partition* /mnt**  
#### 9.3. **mount --mkdir /dev/*efi_system_partition* /mnt/boot**  
#### 9.4. **swapon /dev/*swap_partition***  
...
##### 11.2.1. I have an AMD processor on my computer, so I used **amd-ucode**. If yours has Intel, you could use **intel-ucode**.
...
##### 11.2.4. Nano is very very useful for editing files, as is vim. I added both **nano** and **vim** to the list.
##### 11.2.5. All of the man pages... use **man-db**, **man-pages**, and **texinfo** addons to the command.  
...
### a. SDDM was said on multiple public forums to have this problem and users recommended using nvidia alongside or as a replacement. I installed nvidia to try to mitigate these problems with sddm - It did not work.
### b. Another source on reddit said that sddm might not boot up properly if there wasn't enough space on the disk. I had so little room that I thought this to be plausible, so I deleted a ton of VMs (one of which i didn't know I had) that were stored on my machine - This did not work, but it added 40GB of storage 
### c. Setting to boot into sddm automatically using **systemctl set-default graphical.target** and then **reboot** - This also did not work and I had to revert to a previous snapshot to continue progress.
### d. This is the point where I messaged the professor about the problem. I forgot to create the home directory. This won't happen to you if you just create the home directory where is says to above in step 28. 

## Section 3: Configuring the Desktop Environment  
...
### 31. Sign in with the user you created before. Now you run into a problem - your Computer and Network links on the desktop throw an error "Operation not permitted". Aiden K and [DietPi](https://dietpi.com/forum/t/in-lxqt-desktop-whenever-i-click-on-computer-it-shows-operation-not-supported/13591/3) recommended installing gvfs, which allows apps to access remote resources. In this case, it gives the desktop environment permission to access the terminal.
